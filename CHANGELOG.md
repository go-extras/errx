...
- **`json` attribute values are encoded once instead of twice** ([#33]) — `serializeAttributes` previously ran `json.Marshal` on every attribute value purely to test serializability, discarded the result, then let the outer `Marshal` encode the same value a second time. The single encode — plus the `fmt.Sprintf("%+v", v)` fallback for values that don't encode (e.g. a `func`) — now happens once in a new `SerializedAttr.MarshalJSON`. `SerializedAttr.Value` keeps the caller's original Go value, so `ToSerializedError` consumers can still inspect the concrete value before serialization; `Marshal`/`MarshalIndent` output is byte-for-byte unchanged. Attribute-heavy marshals allocate noticeably less (an 8-scalar-attribute error: 28 → 23 allocs/op; a nested-map/struct error: 97 → 45, ≈−32% bytes) and run faster (≈−8% scalar, ≈−24% nested; M3 Pro, go1.26.3, `benchstat` n=10). Removes the now-unused `isJSONSerializable` helper.
...
[#33]: https://github.com/go-extras/errx/issues/33

...
- **JSON no longer treats empty metadata carriers as sentinels** ([#57]) — `Attrs()`, `FromAttrMap(nil)`, and empty attribute maps no longer emit `"(empty attribute list)"` in `sentinels`. Attribute and trace carriers are excluded by their own type, including external implementations with empty payloads. Actual sentinels retain their text when an attributed or traced parent carries metadata; displayable detection remains chain-wide.

- **Error traversal distinguishes types at the same address** ([#56]) — cycle detection now keys errors by both their type and data words. A cause stored as its wrapper's first field and distinct zero-size error types no longer lose attributes or stack traces or produce a false circular-reference marker in JSON. `ExtractAttrs` tracks typed-nil errors separately by type, preserving distinct nil-safe branches and stopping typed-nil cycles. The larger identity keys increase memory used by visited maps on deep chains; allocation counts are unchanged in the measured benchmarks.
...
[#57]: https://github.com/go-extras/errx/issues/57

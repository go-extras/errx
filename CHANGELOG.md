...
- **`errptr` identity is `(type, data)`, not the data word alone** ([#56]) — `internal/errptr.Get` now returns a comparable `ID` of both interface-header words. Cycle detection keyed only on the data pointer skipped a cause stored as a wrapper's first field (`&outer == &outer.cause`) and collapsed distinct zero-size types that share the runtime zerobase. `HasAttrs` / `ExtractAttrs`, the `stacktrace` walker, and the `json` visited set all use the new ID. Typed-nil / nil still skip tracking (`ID.IsZero`).
